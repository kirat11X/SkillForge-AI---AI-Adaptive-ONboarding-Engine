# SkillForge-AI---AI-Adaptive-On-Boarding-Engine
...
Resume PDF + JD PDF
	...
	-> Module 8: Final Learning Roadmap (Provide explanable results in text format)
	-> JSON outputs for explainable decisioning and UI
	-> Output on User Terminal
```
## Architecture
```mermaid
flowchart LR
    ...
    R --> S([Module 8 Final Learning Roadmap])
```
...
### Module 8: Final Learning Roadmap
Path: `module8/reasoning_engine.py`

Inputs:
- Module 4 gap output (`output/module_4/gapengine_output.json`)
- Module 5 profession mapping output (`output/module_5/profession_mapping_output.json`)
- Module 6 adaptive roadmap output (`output/module_6/adaptive_path_output.json`)

Core approach:
- Synthesizes signals from gap severity, JD importance, profession-fit, and dependency-aware roadmap priority
- Produces an explainable reasoning trace for:
	- JD qualification judgement (`matched`, `qualified`, `not_qualified`)
	- Why specific roadmap skills were prioritized first
	- Why certain professions were selected as best-fit
	- Why critical/moderate gaps were marked
	- Why some skills were deferred in the current roadmap
- Builds both machine-readable JSON reasoning and human-readable text report

Output files:
- `output/module_8/reasoning_trace_output.json`
- `output/module_8/reasoning_trace.txt`

Key output sections (JSON):
- `jd_qualification_assessment`
- `decision_perspective`
- `all_jd_gaps_with_priority`
- `roadmap_reasoning`
- `profession_reasoning`
- `gap_reasoning`
- `contrast_reasoning`
- `report_text`
...
```json
{
	"summary_line": "Every recommendation ... explainable",
	"jd_qualification_assessment": {
		"verdict": "qualified",
		"coverage_ratio": 0.67,
		"summary": "Overall JD judgement: Qualified ...",
		"reasoning_trace": ["..."]
	},
	"decision_perspective": {
		"best_fit_role": "Backend Engineer",
		"target_jd_role": "System Software Engineer",
		"roadmap_policy": "JD alignment first, role support second."
	},
	"all_jd_gaps_with_priority": [
		{
			"rank": 1,
			"skill": "sql",
			"gap_level": "Critical Gap",
			"gap_score": 4.8
		}
	],
	"roadmap_reasoning": [
		{
			"skill": "sql",
			"heading": "SQL selected first because:",
			"reasoning_trace": ["..."]
		}
	],
	"report_text": "Reasoning Trace ..."
}
```

CLI options (optional):
- `--top-k` controls how many top items are included per reasoning section (default: 3)
- `--text-out` controls the rendered text report output path
...
- Resume: `Arnav_Sachdeva_SWE_Intern_Resume.pdf`
- JD: `System Software Engineer.pdf`
