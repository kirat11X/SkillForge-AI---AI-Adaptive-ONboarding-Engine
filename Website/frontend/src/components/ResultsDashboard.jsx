...
function GapComparisonBar({ summary }) {
  if (!summary || !summary.totalSkillsCompared) return null

  const segments = [
    { key: 'matched', label: 'Matched', value: summary.matched, className: 'matched' },
    { key: 'partialMatch', label: 'Partial match', value: summary.partialMatch, className: 'partial' },
    { key: 'missing', label: 'Missing', value: summary.missing, className: 'missing' },
  ...
  return (
    <div className="panel comparison-panel">
      <div className="panel-heading">
        <div>
          <span className="eyebrow">Module 4 comparison</span>
          <h3>Resume vs JD Gap Status</h3>
        </div>
        <p className="panel-caption">
          {summary.totalSkillsCompared} skills compared against the job description.
        </p>
      </div>
      <div className="comparison-bar">
        {segments.map((item) => (
          <div
            key={item.key}
            className={`comparison-segment ${item.className}`}
            style={{ width: `${(item.value / summary.totalSkillsCompared) * 100}%` }}
      ...
      <div className="comparison-legend">
        {segments.map((item) => (
          <div className="comparison-stat" key={item.key}>
        ...
        {!!summary.criticalGap && (
          <div className="comparison-stat muted-stat">
            <strong>{summary.criticalGap}</strong>
            <span>Critical gap</span>
          </div>
        )}
        {!!summary.moderateGap && (
          <div className="comparison-stat muted-stat">
            <strong>{summary.moderateGap}</strong>
...
function ResultsDashboard({ data }) {
  ...
  return (
    <section className="results">
      <GapComparisonBar summary={overview.gapComparison} />
