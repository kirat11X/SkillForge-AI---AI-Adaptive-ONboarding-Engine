...
def _gap_comparison_summary(gap_data: dict) -> dict:
    items = [
        details
        for skill, details in gap_data.items()
        if not skill.startswith("__") and isinstance(details, dict)
    ]
    total = len(items)
    missing = sum(1 for item in items if item.get("status") == "missing")
    partial_match = sum(1 for item in items if item.get("status") == "partial_match")
    matched = sum(1 for item in items if item.get("status") == "matched")
    critical = sum(1 for item in items if item.get("level") == "Critical Gap")
    moderate = sum(1 for item in items if item.get("level") == "Moderate Gap")
    slight = sum(1 for item in items if item.get("level") == "Slight Gap")
    good_match = sum(1 for item in items if item.get("level") == "Good Match")

    return {
        "totalSkillsCompared": total,
        "missing": missing,
        "partialMatch": partial_match,
        "matched": matched,
        "criticalGap": critical,
        "moderateGap": moderate,
        "slightGap": slight,
...
def build_structured_response(
    ...
    gap_summary = _gap_comparison_summary(gap_data)
