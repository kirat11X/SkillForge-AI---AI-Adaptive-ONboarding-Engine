...
def _find_workspace_root() -> Path:
    ...
    for candidate in current_file.parents:
        if (candidate / "Website").is_dir() and (candidate / "pipeline").is_dir():
            return candidate
    raise RuntimeError(
        "Could not locate the project root containing both 'Website' and 'pipeline' directories."
    )
...
DEFAULT_JD_CANDIDATES = (
    PIPELINE_DIR / "System Software Engineer.pdf",
    PIPELINE_DIR / "Machine-Learning-Engineer.pdf",
)
DEFAULT_JD_PATH = next((path for path in DEFAULT_JD_CANDIDATES if path.exists()), DEFAULT_JD_CANDIDATES[0])
TEMP_UPLOADS_DIR = BACKEND_DIR / "uploads"
